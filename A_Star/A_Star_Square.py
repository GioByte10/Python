from SpotSquare import SpotSquare
from tkinter import *
import math
import time
...
cols = 50
grid = []
SpotSquare.rows = rows
SpotSquare.cols = cols

squareHeight = int(c.cget("height")) / rows
squareWidth = int(c.cget("width")) / cols
SpotSquare.height = squareHeight
SpotSquare.width = squareWidth
...
def createSpotGrid():
    ...
    for row in range(rows):
        for col in range(cols):
            grid[row][col] = SpotSquare(row, col)

    SpotSquare.grid = grid
